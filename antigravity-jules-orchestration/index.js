...
import { SubmoduleService } from './lib/submodule-service.js';
...
const submoduleService = new SubmoduleService(path.join(process.cwd(), '..'));
...
app.get('/api/submodules', async (req, res) => {
  try {
    const submodules = await submoduleService.listSubmodules();
    res.json(submodules);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.get('/api/submodules/processes', (req, res) => {
  const processes = submoduleService.getRunningProcesses();
  res.json(processes);
});

app.delete('/api/submodules/processes/:id', (req, res) => {
  const success = submoduleService.killProcess(req.params.id);
  if (success) {
...
app.get('/api/submodules/:name', async (req, res) => {
  try {
    const info = await submoduleService.getSubmoduleInfo(path.join(submoduleService.submodulesDir, req.params.name));
    res.json(info);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.get('/api/submodules/:name/config', async (req, res) => {
  try {
    const { file } = req.query;
    if (!file) return res.status(400).json({ error: 'File parameter required' });
    const content = await submoduleService.getConfigFile(req.params.name, file);
    res.json({ content });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.post('/api/submodules/:name/config', async (req, res) => {
  try {
    const { file, content } = req.body;
    if (!file || content === undefined) return res.status(400).json({ error: 'File and content required' });
    await submoduleService.updateConfigFile(req.params.name, file, content);
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.post('/api/submodules/:name/execute', async (req, res) => {
  try {
    const { script, command } = req.body;
    const result = await submoduleService.executeScript(req.params.name, script, command);
    res.json(result);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
const server = app.listen(PORT, '0.0.0.0', () => {
  ...
  server.on('upgrade', (request, socket, head) => {
    ...
    if (pathname === '/ws') {
      //Existing WS logic if any, or new handler
      // For now, let's assume specific path for execution
      socket.destroy();
    } else if (pathname.startsWith('/ws/submodules/execute')) {
      // Extract params from URL or handle initial message
      // Actually, better to use a single WS endpoint and message types, but for quick implementation:
      // /ws/submodules/execute?name=aichat&script=dev
      const url = new URL(request.url, `http://${request.headers.host}`);
      const submoduleName = url.searchParams.get('name');
      const script = url.searchParams.get('script');
      const command = url.searchParams.get('command'); // Optional override

      if (!submoduleName || (!script && !command)) {
        socket.write('HTTP/1.1 400 Bad Request\r\n\r\n');
        socket.destroy();
        return;
      }

      // We need a way to accept the socket. SubmoduleService needs to handle the raw socket or we wrap it in a WebSocket lib instance.
      // Since 'ws' is used, let's upgrade it.
      const wss = new WebSocketServer({ noServer: true });
      wss.handleUpgrade(request, socket, head, async (ws) => {
        // Resolve command if script is provided
        let cmd = command;
        if (!cmd && script) {
          const info = await submoduleService.getSubmoduleInfo(path.join(submoduleService.submodulesDir, submoduleName));
          cmd = info.scripts[script];
        }

        if (!cmd) {
          ws.send(JSON.stringify({ type: 'error', data: 'Script not found' }));
          ws.close();
          return;
        }

        submoduleService.executeScriptStream(submoduleName, script, cmd, ws);
      });
    } else {
      socket.destroy();
    }
