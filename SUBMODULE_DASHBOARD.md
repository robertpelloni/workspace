# Omni-Workspace Advanced Dashboard
**Last Updated:** 2026-03-24 13:07:04
**Project Version:** 1.6.1

## Project Directory Structure
The workspace is organized into several functional clusters:
- **Root**: Core orchestration scripts, configuration, and global documentation.
- **`antigravity-autopilot/`**: Autonomous agent frameworks and automation tools.
- **`bg/`**: The "bob's game" ecosystem, including engine components and legacy references.
- **`bobmani/`**: Rhythm game ecosystem (StepMania/ITGmania themes and tools).
- **`bobsaver/`**: Visualizer ecosystem (MilkDrop, JWildfire, projectM).
- **`docs/`**: Unified project requirements, design, and research documentation.
- **`scripts/`**: Automation, synchronization, and maintenance utilities.

## AI Contribution Summary
- **AI-Generated Commits:** 2589 (0.4% of total)
- **AI-Added Lines:** 24,141,322
- **AI-Deleted Lines:** 22,534,144

## Submodule Inventory (50 total)

| Submodule | Version | Status | AI% | Commit | Date | Latest Change |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **.agent** | `8.5.0` | ⏳ Pending | 21.7% | `03b5fdc0` | 2026-03-21 | Merge branch 'main' of https://github... |
| **antigravity-autopilot** | `6.2.59` | ✅ Success | 0.2% | `800561b0` | 2026-03-20 | chore: force prune broken submodules |
| **bg** | `2.0.0` | ⏳ Pending | 0.0% | `5fa73ae9` | 2026-03-21 | chore: save progress before update |
| **bobbybookmarks** | `N/A` | ⏳ Pending | 16.7% | `d4bb056` | 2026-03-23 | chore: reorganize project structure a... |
| **bobcoin** | `N/A` | ✅ Success | 19.8% | `f841c92` | 2026-03-20 | chore: prune broken nested submodules |
| **bobdesk** | `N/A` | ⏳ Pending | 0.0% | `952f614762bf` | 2026-02-26 | Fix submodule URLs to bypass Jules mi... |
| **bobeditpro** | `N/A` | ⏳ Pending | 6.7% | `dcc7c9242` | 2026-03-21 | chore: save progress before update |
| **bobfilez** | `2.4.1` | ✅ Success | 12.6% | `32690ba6` | 2026-03-21 | chore: save progress before update |
| **bobium** | `N/A` | ⏳ Pending | 0.0% | `fa67492` | 2026-02-26 | chore: save progress before update |
| **bobmani/arrowvortex** | `v1.3.2` | ✅ Success | 25.4% | `4e073c3` | 2026-03-05 | added odcnn to arrowvortex |
| **bobmani/beatoraja** | `0.9.2` | ⏳ Pending | 1.9% | `012d9ed7` | 2026-03-21 | chore: save progress before update |
| **bobmani/bobmania** | `5.7.0-Unified-Alpha` | ⏳ Pending | 0.2% | `161dc98162` | 2026-03-21 | chore: save progress before update |
| **bobmani/ddc** | `0.2.1` | ✅ Success | 69.7% | `d1241fd` | 2026-03-17 | chore: save progress before update |
| **bobmani/ddc_onset** | `N/A` | ✅ Success | 0.0% | `ffc1f53` | 2026-03-01 | chore: save progress before update |
| **bobmani/ffr-difficulty-model** | `N/A` | ✅ Success | 75.0% | `90ac515` | 2026-03-01 | chore: save progress before update |
| **bobmani/hymnmania** | `N/A` | ⏳ Pending | 78.6% | `b72c55f` | 2026-03-19 | feat: Add UI robustness, file downloa... |
| **bobmani/itgmania** | `1.3.0` | ✅ Success | 0.0% | `40219a2462` | 2026-03-21 | chore: save progress before update |
| **bobmani/ksm-v2** | `2.0.0-alpha23` | ✅ Success | 2.0% | `045006d` | 2026-03-21 | Merge remote-tracking branch 'origin/... |
| **bobmani/leraine-studio** | `N/A` | ✅ Success | 25.4% | `fa58333` | 2026-03-01 | chore: save progress before update |
| **bobmani/linthesia** | `0.9.0` | ⏳ Pending | 1.8% | `1a62e78` | 2026-02-28 | chore: save progress before update |
| **bobmani/pianogame** | `N/A` | ⏳ Pending | 0.0% | `5860035` | 2026-02-27 | Merge branch 'master' of https://gith... |
| **bobmani/Simply-Love-SM5** | `N/A` | ⏳ Pending | 0.1% | `5c23b0ba` | 2026-02-26 | Merge branch 'itgmania-release' of ht... |
| **bobsaver** | `N/A` | ⏳ Pending | 0.0% | `34f55ea` | 2026-03-21 | chore: save progress before update |
| **bobtorrent** | `11.2.4` | ✅ Success | 18.7% | `c71fa17` | 2026-03-21 | chore: save progress before update |
| **bobtrader** | `2.0.0` | ⏳ Pending | 1.7% | `aa67b11` | 2026-03-11 | chore: save progress before update |
| **bobtrax** | `N/A` | ⏳ Pending | 0.0% | `f40a62d` | 2026-03-17 | chore: save progress before update |
| **bobui** | `N/A` | ✅ Success | 0.0% | `06af9492316` | 2026-03-01 | chore: save progress before update |
| **bobzilla** | `N/A` | ⏳ Pending | 0.0% | `e65decc` | 2026-03-01 | chore: save progress before update |
| **bobzzite** | `N/A` | ⏳ Pending | 0.0% | `fa1236f` | 2026-01-12 | Initialize bobzzite gaming Linux dist... |
| **borg** | `0.99.3` | ⏳ Pending | 19.2% | `5a7cd833` | 2026-03-24 | docs: Flag sprint 0.99.4 checklist it... |
| **dupeguru** | `{attr` | ⏳ Pending | 0.0% | `c0df9eb1` | 2026-03-11 | Merge remote-tracking branch 'origin/... |
| **f-zerox** | `0.1.16` | ⏳ Pending | 48.4% | `f20b10a` | 2026-03-23 | build: Add CMakeLists.txt for modern ... |
| **frontend-sdl-cpp** | `N/A` | ⏳ Pending | 0.0% | `bc3c0ae` | 2026-03-11 | Merge remote-tracking branch 'origin/ci' |
| **fwber** | `0.6.9-beta` | ⏳ Pending | 9.1% | `e7c87158d` | 2026-03-23 | feat: implement ActivityPub Dating Na... |
| **jules-autopilot** | `0.9.2` | ⏳ Pending | 22.0% | `1563762` | 2026-03-21 | chore: save progress before update |
| **Maestro** | `0.15.3` | ⏳ Pending | 0.7% | `017dfb47` | 2026-03-21 | Merge pull request #603 from scriptea... |
| **MarbleBlast** | `1.0.0` | ⏳ Pending | 0.0% | `b4880e4` | 2025-09-26 | Merge pull request #30 from Abhiguy/p... |
| **mcp-superassistant** | `0.7.2` | ⏳ Pending | 9.5% | `26b7136` | 2026-03-10 | feat(core): borg assimilation rename ... |
| **mk64** | `N/A` | ⏳ Pending | 0.0% | `356bd335b` | 2026-03-21 | Merge remote-tracking branch 'upstrea... |
| **neverball** | `1.6.2-dev` | ⏳ Pending | 4.4% | `464de077` | 2026-03-21 | chore: save progress before update |
| **npp** | `N/A` | ⏳ Pending | 0.1% | `62af7b605` | 2026-03-21 | Merge pull request #1 from robertpell... |
| **OmniRoute** | `2.9.4` | ⏳ Pending | 4.6% | `ed75289` | 2026-03-21 | Merge branch 'main' of https://github... |
| **opencode-autopilot** | `1.0.14` | ⏳ Pending | 21.5% | `d6b2001` | 2026-03-21 | chore: save progress before update |
| **OpenMBU** | `N/A` | ⏳ Pending | 0.0% | `3ff94584` | 2026-01-18 | Update LICENSE.md |
| **picard** | `0.4.0` | ⏳ Pending | 0.7% | `ec6084893` | 2026-03-23 | feat: merge Soulseek Downloader ideas... |
| **raindropioapp** | `1.0.4` | ⏳ Pending | 9.1% | `8d101093` | 2026-03-21 | Merge remote-tracking branch 'upstrea... |
| **sm64coopdx** | `v1.9` | ⏳ Pending | 0.9% | `2f383f29f` | 2026-03-21 | chore: save progress before update |
| **superpowers** | `N/A` | ⏳ Pending | 1.0% | `8123184` | 2026-03-21 | Merge remote-tracking branch 'origin/... |
| **supersaber** | `1.0.0` | ⏳ Pending | 0.0% | `e8d7292` | 2018-12-11 | add to site, not an official project |
| **topaz-ffmpeg** | `N/A` | ⏳ Pending | 0.0% | `295c219d2a` | 2026-02-21 | chore: save progress before update |
