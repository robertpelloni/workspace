...
if (Test-Path $codexConfig) {
    ...
    if ($content -like "*OPENAI_API_KEY*" -and $content -notlike "*OPENAI_API_KEY = ""*") {
        Write-Host "   OK - TOML syntax appears valid" -ForegroundColor Green
        Write-Host "   OK - API keys are properly configured" -ForegroundColor Green
    } else {
        Write-Host "   WARNING - API keys may be missing or empty" -ForegroundColor Yellow
    }
} else {
...
if (Test-Path $grokConfig) {
    try {
        ...
        Write-Host "   OK - Zen MCP Server configured" -ForegroundColor Green
    } catch {
        Write-Host "   ERROR - JSON syntax error: $($_.Exception.Message)" -ForegroundColor Red
    }
} else {
    Write-Host "   ERROR - Config file not found" -ForegroundColor Red
}
