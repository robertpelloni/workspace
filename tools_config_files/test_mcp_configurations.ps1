...
if (Test-Path $grokConfig) {
    try {
        ...
        if ($grokContent.mcpServers.'zen-mcp-server'.env.OPENAI_API_KEY) {
...
if (Test-Path $claudeConfig) {
    try {
        ...
        if ($claudeContent.mcpServers.'zen-mcp-server'.env.OPENAI_API_KEY) {
            Write-Host "   ✅ JSON syntax is valid" -ForegroundColor Green
            Write-Host "   ✅ Zen MCP Server environment variables configured" -ForegroundColor Green
        } else {
            Write-Host "   ⚠️  Zen MCP Server environment variables may be missing" -ForegroundColor Yellow
...
if (Test-Path $clineConfig) {
    try {
        ...
        if ($clineContent.mcpServers.'zen-mcp-server'.env.OPENAI_API_KEY) {
            Write-Host "   ✅ JSON syntax is valid" -ForegroundColor Green
            Write-Host "   ✅ Zen MCP Server environment variables configured" -ForegroundColor Green
        } else {
            Write-Host "   ⚠️  Zen MCP Server environment variables may be missing" -ForegroundColor Yellow
...
if (Test-Path $geminiConfig) {
    try {
        ...
        if ($geminiContent.mcpServers.'zen-mcp-server'.env.OPENAI_API_KEY) {
            Write-Host "   ✅ JSON syntax is valid" -ForegroundColor Green
            Write-Host "   ✅ Zen MCP Server environment variables configured" -ForegroundColor Green
        } else {
            Write-Host "   ⚠️  Zen MCP Server environment variables may be missing" -ForegroundColor Yellow
...
if (Test-Path $copilotConfig) {
    try {
        ...
        if ($copilotContent.mcpServers.'zen-mcp-server'.env.OPENAI_API_KEY) {
            Write-Host "   ✅ JSON syntax is valid" -ForegroundColor Green
            Write-Host "   ✅ Zen MCP Server environment variables configured" -ForegroundColor Green
        } else {
            Write-Host "   ⚠️  Zen MCP Server environment variables may be missing" -ForegroundColor Yellow
