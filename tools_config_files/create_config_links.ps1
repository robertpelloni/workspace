# PowerShell script to create symbolic links for all MCP configuration files
...
Write-Host "Creating symbolic links for MCP configuration files..." -ForegroundColor Green
...
foreach ($linkName in $configFiles.Keys) {
    ...
    if (Test-Path $targetPath) {
        ...
        if (Test-Path $linkName) {
            ...
            Write-Host "Removed existing: $linkName" -ForegroundColor Yellow
        }
        
        try {
            # Create symbolic link
            New-Item -ItemType SymbolicLink -Path $linkName -Target $targetPath
            Write-Host "Created link: $linkName -> $targetPath" -ForegroundColor Green
        }
        catch {
            Write-Host "Failed to create link for $linkName : $($_.Exception.Message)" -ForegroundColor Red
        }
    }
    else {
...
Write-Host "`nSymbolic link creation completed!" -ForegroundColor Green
