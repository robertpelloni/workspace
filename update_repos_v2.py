...
PROCESSED_FILE = "processed_repos_v2.txt"
FAILED_LOG = "failed_repos_v2.log"
...
def process_repo(name, cwd, processed_set):
    ...
    if remotes and "upstream" in remotes.split():
        ...
        try:
            merge_res = run_command(f"git merge upstream/{default_branch}", cwd, timeout=120)
    ...
    if current_branch != default_branch and current_branch != "HEAD":
        ...
        try:
            merge_res = run_command(f"git merge {current_branch}", cwd, timeout=120)
            if merge_res is None:
                 raise Exception("Merge failed")
        except:
             msg = f"Feature merge failed for branch {current_branch}. Aborting."
             print(msg)
             log_failure(cwd, msg)
             run_command("git merge --abort", cwd, ignore_errors=True)
