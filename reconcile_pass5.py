#!/usr/bin/env python3
"""
Reconciliation Pass 5 — Fresh fetch, find new unmerged feature branches.
v5.251.0 → v5.252.0
"""
...
LOG_FILE = WORKSPACE / "merge_pass5.log"
...
def main():
    try:
        with open(LOG_FILE, "w", encoding="utf-8") as f:
            f.write(f"=== RECONCILIATION PASS 5 — {datetime.now()} ===\n\n")
    except OSError:
        print("Warning: Could not write log file")

    log(f"Starting Reconciliation Pass 5 from {WORKSPACE}")
    ...
    log(f"\n{'=' * 60}")
    log("RECONCILIATION PASS 5 SUMMARY")
    log(f"{'=' * 60}")
