#!/usr/bin/env python3
"""
Reconciliation Pass 6 — Fresh fetch, find new unmerged feature branches.
v5.252.0 → v5.253.0
"""
...
LOG_FILE = WORKSPACE / "merge_pass6.log"
STATE_FILE = WORKSPACE / "merge_pass6_state.json"
    ...
    for branch in branches:
        ...
        ok, _, err = run(
            [
                ...
                "--no-edit",
                "-m",
                f"merge: {branch} into {default_branch} (pass 6)",
            ],
            cwd=full_path,
            timeout=120,
        )
...
def main():
    try:
        with open(LOG_FILE, "w", encoding="utf-8") as f:
            f.write(f"=== RECONCILIATION PASS 6 — {datetime.now()} ===\n\n")
    except OSError:
        print("Warning: Could not write log file")

    log(f"Starting Reconciliation Pass 6 from {WORKSPACE}")
    ...
    log(f"\n{'=' * 60}")
    log("RECONCILIATION PASS 6 SUMMARY")
    log(f"{'=' * 60}")
