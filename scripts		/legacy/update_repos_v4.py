...
PROCESSED_FILE = "processed_repos_v4.txt"
FAILED_LOG = "failed_repos_v4.log"
...
def process_repo(name, cwd, processed_set):
    ...
    # 5. Merge remote feature branches for robertpelloni repos
    ...
    # 6. Push to Origin (only if it's a robertpelloni repo or fork)
    remotes_v = run_command("git remote -v", cwd, ignore_errors=True)
    if remotes_v and "robertpelloni" in remotes_v.lower():
