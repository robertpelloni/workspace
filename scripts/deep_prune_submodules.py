...
def audit_dir(target_dir):
    ...
    if not (os.path.isdir(os.path.join(target_dir, ".git")) or os.path.isfile(os.path.join(target_dir, ".git"))):
        return
    ...
    # Walk all subdirectories to find more git repos (nested submodules)
    for entry in os.listdir(target_dir):
        full_entry_path = os.path.join(target_dir, entry)
        if os.path.isdir(full_entry_path) and entry != ".git":
